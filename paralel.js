...
const gasPrice = ethers.parseUnits('0.000000001900000059', 'ether');
...
const fetchUserData = async (amigo) => {
  const userDataResponse = await axios.get(`https://prod-api.kosetto.com/users/${amigo}`);
  return userDataResponse.data;
};

const fetchTwitterFollowerCount = async (twitterUsername) => {
  return new Promise((resolve, reject) => {
    twitterClient.get('users/show', { screen_name: twitterUsername }, (error, twitterUser) => {
      if (error) {
        reject(error);
      } else {
        resolve(twitterUser.followers_count);
      }
    });
  });
};
...
const run = async () => {
  ...
  friends.on(filter, async (event) => {
    if (event.args[2] == true) {
      ...
      try {
        const userData = await fetchUserData(amigo);
        const followerCount = await fetchTwitterFollowerCount(userData.twitterUsername);

        console.log(`[${currentTime}] Twitter Followers for ${userData.twitterUsername}: ${followerCount}`);
        ...
        console.error(`[${currentTime}] Error: ${error}`);
      }
    }
  });
}
